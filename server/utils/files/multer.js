...
const { ensurePodcastTranscriptStorage } = require("./podcastTranscripts");
...
/**
 * Handle podcast transcript uploads
 */
const podcastTranscriptStorage = multer.diskStorage({
  destination: function (_, __, cb) {
    const uploadOutput = ensurePodcastTranscriptStorage();
    cb(null, uploadOutput);
  },
  filename: function (_, file, cb) {
    file.originalname = normalizePath(
...
function handlePodcastTranscriptUpload(request, response, next) {
  const upload = multer({ storage: podcastTranscriptStorage }).array(
    "files",
    20
  );
  upload(request, response, function (err) {
    if (err) {
      response
        ...
        .json({
          success: false,
          error: `Invalid transcript upload. ${err.message}`,
        })
        .end();
      return;
    }
