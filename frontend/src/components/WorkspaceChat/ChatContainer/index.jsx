...
export default function ChatContainer({ workspace, knownHistory = [] }) {
  ...
  const [autoAgentBooted, setAutoAgentBooted] = useState(false);
  ...
  useEffect(() => {
    if (!workspace?.slug || autoAgentBooted) return;
    setAutoAgentBooted(true);
    const initializeSession = async () => {
      // First clear any residual chat state in the workspace
      await sendCommandRef.current({ text: "/reset", autoSubmit: true });
      // Follow up with agent invocation once reset command is dispatched
      setTimeout(() => {
        sendCommandRef.current({ text: "@agents", autoSubmit: true });
      }, 250);
    };
    initializeSession();
  }, [workspace?.slug, autoAgentBooted]);
