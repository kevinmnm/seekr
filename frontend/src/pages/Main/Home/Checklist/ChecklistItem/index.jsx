...
import { ASSESSMENT_DISABLED_TOOLTIP_ID } from "@/utils/assessment";
...
export function ChecklistItem({
  ...
  return (
    <div
      {...tooltipProps}
      className={`flex items-center gap-x-4 transition-colors rounded-lg p-3 group hover:bg-theme-checklist-item-bg-hover ${
        isCompleted
    ...
    >
      ...
      {isCompleted ? (
        ...
        <button
          type="button"
          disabled={disabled}
          className={`w-[64px] h-[24px] rounded-md bg-white/10 light:bg-white/70 text-theme-checklist-item-text font-semibold text-xs transition-all duration-200 flex items-center justify-center ${
            disabled
              ? "opacity-40 cursor-not-allowed pointer-events-none"
              : "hover:bg-white/20 light:hover:bg-white/60"
          }`}
        >
          {action}
        </button>
